...
def fasta2phylip(fastafn, codenames=False, shuffle=False):
    """Transforms a FASTA-formatted file in PHYLIP format.
    """
    ...
    with open(fastafn) as fastaf:
        ...
        for line in fastaf:
            if line.startswith(">"):
                ...
                sname = line[1:].strip()
                if sname in snames:
                    print(("WARNING: name {0} is duplicated.\n"
                        "Sequence #{1} will not be considered")\
                        .format(sname, iseq))
                    jump = True
                    continue
                snames.append(sname)
                ...
                seqs[sname] += line.strip() # Writes all! does not make any selection
